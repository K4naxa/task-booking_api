/**
 * Integration Test Suite for Room Reservation API
 *
 * This file tests the production server by sending actual HTTP requests.
 * Make sure the server is running before executing these tests.
 *
 * Usage: npx tsx test-api.ts
...
// Helper functions to get future times with 10-minute granularity
...
async function testCreateBooking() {
  ...
  assert(
    response.data.userId === 'testuser123',
    'Booking should have correct userId',
  );
  ...
  return response.data.id; // Return booking ID for later tests
}
...
async function testBookingValidations() {
  ...
  // Test 1: Invalid userId (non-alphanumeric)
  ...
  assert(
    invalidUserIdResponse.status === 400,
    'Should reject invalid userId format',
  );
  assert(
    JSON.stringify(invalidUserIdResponse.data).includes('alphanumeric'),
    'Error should mention alphanumeric requirement',
  );

  // Test 2: Past startTime
  ...
  assert(
    JSON.stringify(pastTimeResponse.data).includes('future'),
    'Error should mention future requirement',
  );

  // Test 3: startTime >= endTime
  ...
  assert(
    invalidTimeResponse.status === 400,
    'Should reject when startTime >= endTime',
  );
  assert(
    JSON.stringify(invalidTimeResponse.data).includes('before'),
    'Error should mention time ordering',
  );

  // Test 4: Invalid time granularity (not 10-minute intervals)
  ...
  invalidGranularityTime.setMinutes(15); // Set to :15 (invalid)
  ...
  assert(
    granularityResponse.status === 400,
    'Should reject invalid time granularity',
  );
  assert(
    JSON.stringify(granularityResponse.data).includes('10-minute'),
    'Error should mention 10-minute intervals',
  );

  // Test 5: Non-existent room
  ...
  assert(
    invalidRoomResponse.status === 404,
    'Should return 404 for non-existent room',
  );

  // Test 6: Overlapping bookings
  ...
  assert(
    overlapResponse.status === 409,
    'Should return 409 for overlapping booking',
  );
  assert(
    JSON.stringify(overlapResponse.data).includes('conflict') ||
      JSON.stringify(overlapResponse.data).includes('overlap'),
    'Error should mention conflict or overlap',
  );

  // Test 7: Non-overlapping bookings for different rooms
  ...
  assert(
    differentRoomResponse.status === 201,
    'Should allow same time for different rooms',
  );

  // Test 8: Valid 10-minute intervals
  ...
  assert(
    validBooking.status === 201,
    'Should accept valid 10-minute intervals',
  );
}
...
async function testCancelBooking() {
  ...
  assert(
    cancelResponse.data.status === 'CANCELLED',
  ...
  const doubleCancelResponse = await makeRequest(
    'PATCH',
    `/bookings/${bookingId}/cancel`,
    { userId: 'canceluser123' },
  );
  assert(
    doubleCancelResponse.status === 409,
    'Should return 409 for already cancelled',
  );
  ...
  const wrongUserResponse = await makeRequest(
    'PATCH',
    `/bookings/${createResponse2.data.id}/cancel`,
    { userId: 'wronguser' },
  );
  assert(
    wrongUserResponse.status === 404,
    'Should return 404 for wrong userId',
  );
  ...
  assert(
    invalidUserResponse.status === 400,
    'Should return 400 for invalid userId format',
  );

  // Test 6: Try to cancel past booking
  log('\nTest: Cancel booking in the past', 'warn');
  const pastBookingStart = getFutureTime(10); // Very near future
  ...
  // Wait a bit and try to cancel (this test might be flaky)
  log('  Note: This test depends on timing and may occasionally fail', 'warn');

  // Test 7: Cancelled bookings don't block time slots
  ...
  assert(
    newBookingResponse.status === 201,
    'Cancelled booking should not block time slot',
  );
}
...
async function testGetUserBookings() {
  ...
  assert(
    response.data[0].room !== undefined,
    'Should include nested room data',
  );

  // Check ordering by startTime
  ...
  // Test 2: Invalid userId format
  log('\nTest: Invalid userId format', 'warn');
  const invalidResponse = await makeRequest(
    'GET',
    '/bookings/user/invalid@user',
  );
  assert(
...
async function testGetRoomBookings() {
  ...
  // Create bookings for the room (confirmed and cancelled)
  ...
  assert(
    allResponse.data[0].room !== undefined,
    'Should include nested room data',
  );

  // Test 2: Get confirmed bookings only
  log('\nTest: GET /bookings/room/:roomId/confirmed', 'warn');
  const confirmedResponse = await makeRequest(
    'GET',
    `/bookings/room/${roomId}/confirmed`,
  );
  ...
  const allConfirmed = confirmedResponse.data.every(
    (b: Booking) => b.status === 'CONFIRMED',
  );
  assert(allConfirmed, 'All bookings should have CONFIRMED status');

  // Test 3: Get cancelled bookings only
  log('\nTest: GET /bookings/room/:roomId/cancelled', 'warn');
  const cancelledResponse = await makeRequest(
    'GET',
    `/bookings/room/${roomId}/cancelled`,
  );
  ...
  assert(
    cancelledResponse.data.length >= 1,
    'Should have at least 1 cancelled booking',
  );

  const allCancelled = cancelledResponse.data.every(
  ...
  // Test 5: Room with no bookings
  log('\nTest: Room with no bookings returns empty array', 'warn');
  const emptyResponse = await makeRequest('GET', '/bookings/room/10');
  assert(emptyResponse.status === 200, 'Should return 200');
  assert(Array.isArray(emptyResponse.data), 'Should return array');
  // Note: Room 10 might have bookings from other tests, so we just check it's an array

  // Test 6: Verify ordering by startTime
